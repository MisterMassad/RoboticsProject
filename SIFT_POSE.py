...
def match_descriptors(desc1, desc2,
    ...
    if use_adaptive and len(knn12) > 0:
        ...
        if debug:
            print(f"[ADAPTIVE] ratio={effective_ratio:.3f}")
    else:
    ...
    if len(one_way) < min_mutual:
        if debug:
            print(f"[MATCH] skip mutual (one_way={len(one_way)} < {min_mutual})")
        return one_way
    ...
    if len(mutual) < 40:
        if debug:
            print(f"[MATCH] mutual starved (mutual={len(mutual)} < {min_mutual}) -> fallback one_way={len(one_way)}")
        return one_way
...
def estimate_pose_two_images(img1_bgr, img2_bgr, K,
    ...
    img1c, g1, s1 = preprocess_bgr(img1_bgr, target_area=pixels)
    img2c, g2, s2 = preprocess_bgr(img2_bgr, target_area=pixels)

...
def estimate_pose_two_images(img1_bgr, img2_bgr, K,
    ...
    # If images are same size and same max_width rule applied, s1 should equal s2
    ...
    if debug:
        print(f"[MATCH] kpts=({len(kpts1)},{len(kpts2)}) matches={len(matches)}")
    ...
    R, t, _ = estimate_pose_from_points(
        pts1, pts2, K_use,
        prob=prob,
        threshold=threshold,
...
def main():
    ...
    R, t = estimate_pose_two_images(
        img1_bgr, img2_bgr, K,
        max_width=900,          # We can disbale resizing by setting this to None
        nfeatures=5000,
        ratio_thresh=0.75,
        max_matches=500,
        use_symmetry=True,

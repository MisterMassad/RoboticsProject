...
# Helper Function: Adaptive matching
def adaptive_ratio_test(knn_matches, image1_gray=None, image2_gray=None):
    """
    Intelligent ratio threshold selection
    """
    # Base ratio from median distance (Method 1)
    distances = []
    ...
    # Dynamic mapping
    if median_dist < 100:
        base_ratio = 0.55  # Excellent matches → be strict
        ...
        base_ratio = 0.90  # Poor matches → be lenient
    
    # Adjust based on spread (Method 2)
    std_dist = np.std(distances)
    cv = std_dist / (median_dist + 1e-6)
    
    if cv > 0.7:  # Highly variable quality
        base_ratio += 0.05  # Be more lenient
    elif cv < 0.2:  # Very consistent quality
        base_ratio -= 0.05  # Be more strict
    
    # Clamp to reasonable bounds
    base_ratio = np.clip(base_ratio, 0.5, 0.95)
    
    return base_ratio

def match_with_adaptive_ratio(desc1, desc2, debug=True):
    bf = cv2.BFMatcher(cv2.NORM_L2)
    
    # Get initial matches for analysis
    knn_matches = bf.knnMatch(desc1, desc2, k=2)
    
    # Compute adaptive ratio
    ratio = adaptive_ratio_test(knn_matches)
    
    if debug:
        print(f"[ADAPTIVE] Using ratio threshold: {ratio:.3f}")
    
    # Apply the adaptive ratio
    good_matches = []
    for pair in knn_matches:
        if len(pair) < 2:
            continue
...
def match_descriptors(desc1, desc2, ratio_thresh=0.75, max_matches=500, 
                     use_symmetry=True, use_adaptive=True, debug=True):
    """
    Modified to support adaptive ratio threshold
    use_adaptive: If True, uses adaptive_ratio_test() to determine threshold
    """
    ...
    # Get initial knn matches for analysis
    ...
    if use_adaptive and len(knn12) > 0:
        ...
        if debug:
            print(f"[ADAPTIVE] Using dynamic ratio: {effective_ratio:.3f} (instead of {ratio_thresh})")
    else:
        effective_ratio = ratio_thresh
        if debug:
            print(f"[MATCH] Using fixed ratio: {effective_ratio}")

    # 1 -> 2 with determined ratio
    good12 = []
    for pair in knn12:
        if len(pair) < 2:
            continue
    ...
    if debug:
        print(f"[MATCH] ratio pass 1->2: {len(good12)}/{len(knn12)} (ratio={effective_ratio:.3f})")
    ...
    # 2 -> 1 (need fresh knn matches for this direction)
    knn21 = bf.knnMatch(desc2, desc1, k=2)
    
    # For symmetry check, we need consistent ratio
    # Could compute adaptive ratio for this direction too, but simpler to use same
    good21 = []
    for pair in knn21:
        if len(pair) < 2:
            continue
